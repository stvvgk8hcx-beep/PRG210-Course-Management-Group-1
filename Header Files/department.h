...
// added const to these as well
        int getCourseCount() const; // display the number of courses
        Course* getCourse(int index) const; // this will come
        // now returns Course as a pointer like the prof CVS
};

// Now uses pragma once instead of ifndef define endif to save lots of space and such 
